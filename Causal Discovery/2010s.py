...
while i < totaltime:
    sym1.append((tc1.variables['varcenter'][i]-tc1.variables['varmin'][i][30])/(tc1.variables['varcenter'][i]-tc1.variables['varmax'][i][30]))
    i += 1

while j < totaltime:
    sym2.append((tc2.variables['varcenter'][j]-tc2.variables['varmin'][j][30])/(tc2.variables['varcenter'][j]-tc2.variables['varmax'][j][30]))
    j += 1


t = [i for i in range(totaltime)]            
pl.plot(t,sym1,label = fname1)
pl.plot(t,sym2,"--",label = fname2)
pl.title("Symmetry when r=10 %")
pl.xlabel("Time")
pl.ylabel("Symmetry")
