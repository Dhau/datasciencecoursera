...
totaltime = len(tc1.variables['slpcenter'])-10
...
while k < 34:
    ...
    j = 10
    while i < totaltime:
        sym1.append((tc1.variables['slpcenter'][i]-tc1.variables['slpmin'][i][k])/(tc1.variables['slpcenter'][i]-tc1.variables['slpmax'][i][k]))
        i += 1
    
    while j < totaltime:
        sym2.append((tc2.variables['slpcenter'][j]-tc2.variables['slpmin'][j][k])/(tc2.variables['slpcenter'][j]-tc2.variables['slpmax'][j][k]))
        j += 1
...
pl.title("Information flow at different r (saomai & bopha)")
pl.xlabel("r")
